...
assert_fail() {
 ...
 FAIL=$((FAIL + 1))
 FAILED_TESTS+=("$name")
 echo -e " ${RED}[FAIL]${R} [$PASS+$FAIL] $name"
 [[ -n "$detail" ]] && echo -e " ${RED}$detail${R}"
}
assert_warn() {
 local name="$1"; local detail="${2:-}"
 WARN=$((WARN + 1))
 echo -e " ${Y}[WARN]${R} $name"
...
#
# [WARN] V5 İKİ AYRI MODEL — bkz docs/MODEL_VERSIONS_TIMELINE.md " KRİTİK UYARI"
...
if [ $FAIL -eq 0 ]; then
 echo -e "${G}${B}╔══════════════════════════════════════════════════════════════════╗${R}"
 echo -e "${G}${B}║ [OK] TÜM TESTLER GEÇTİ! Full ablation başlatılabilir. ║${R}"
 echo -e "${G}${B}╚══════════════════════════════════════════════════════════════════╝${R}"
 echo ""
 ...
 echo -e "${RED}${B}╔══════════════════════════════════════════════════════════════════╗${R}"
 echo -e "${RED}${B}║ [FAIL] $FAIL TEST BAŞARISIZ! Önce hataları düzelt. ║${R}"
 echo -e "${RED}${B}╚══════════════════════════════════════════════════════════════════╝${R}"
 echo ""
