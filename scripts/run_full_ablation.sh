...
#
# ⚠️  V5 IS TWO SEPARATE MODELS — see docs/MODEL_VERSIONS_TIMELINE.md
...
for cont in a4_ollama a4_sim; do
    ...
    if [ "$status" = "running" ]; then
        echo -e "  ${G}✓${R} $cont running"
    else
        echo -e "  ${RED}✗${R} $cont NOT running ($status) — aborting"
        exit 2
    fi
done
...
if ! docker exec a4_sim pgrep -f gzclient > /dev/null 2>&1; then
    echo ""
    echo -e "  ${Y}⚠ gzclient not running — sandbox will execute but you won't see the arm.${R}"
...
for m in "${MODELS[@]}"; do
    if docker exec a4_ollama ollama show "$m" --modelfile > /dev/null 2>&1; then
        ...
        echo -e "  ${RED}✗${R} $m MISSING"
        MISSING+=("$m")
    fi
done
if [[ ${#MISSING[@]} -gt 0 ]]; then
    echo ""
    echo -e "${RED}❌ Missing models: ${MISSING[*]}${R}"
...
if [ "$FREE_GB" -lt 5 ]; then
    echo -e "${RED}❌ Low disk (<5 GB). Aborting to prevent mid-run failure.${R}"
    exit 4
fi
...
if [ $EXIT -eq 0 ]; then
    ...
    echo -e "${B}${Y}║   ⚠ ABLATION INTERRUPTED — partial results saved                   ║${R}"
else
    echo -e "${B}${RED}║   ❌ ABLATION FAILED (exit=$EXIT)                                     ║${R}"
fi
echo -e "${B}${M}╚════════════════════════════════════════════════════════════════════╝${R}"
echo ""
