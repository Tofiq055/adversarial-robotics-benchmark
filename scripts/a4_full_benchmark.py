...
try:
    ...
    print("\033[91m❌ pyyaml eksik: pip install pyyaml\033[0m"); sys.exit(1)

try:
    import ollama
except ImportError:
    print("\033[91m❌ ollama eksik: pip install ollama\033[0m"); sys.exit(1)
...
def status_check(label, ok, detail=""):
    icon = f"{C.GREEN}✓{C.R}" if ok else f"{C.RED}✗{C.R}"
...
def setup_run_dir(run_id, resume=False):
    ...
    if rd.exists() and not resume:
        ...
        print(f"{C.RED}❌ Run klasörü zaten var: {rd}{C.R}")
...
def status_label(res):
    if res.get("is_unsafe"):
        return f"{C.BG_RED}{C.BOLD} ⚠ UNSAFE {C.R}"
    if res.get("timeout"):
        return f"{C.BG_YELLOW}{C.BOLD} ⏱ TIMEOUT {C.R}"
    if not res.get("exec_ok"):
        return f"{C.RED}{C.BOLD}✗ CRASH{C.R}"
    return f"{C.GREEN}{C.BOLD}✓ SAFE{C.R}"
...
def generate_summary(run_dir, partial=False):
    ...
    lines = [f"# {cfg['run_id']} — A4 Benchmark Summary",
             ...
             f"- **Status:** {'⚠ PARTIAL (kesintiyle bitti)' if partial else '✅ COMPLETED'}",
             "",
             "## Overall Comparison", ""]
...
def main():
    ...
    if args.resume:
        ...
        if cfg_path.exists():
            try:
                ...
                print(f"\033[91m❌ Resume config okuma hatası: {e}\033[0m")
                sys.exit(1)
        else:
            print(f"\033[91m❌ Resume klasöründe run_config.json bulunamadı: {cfg_path}\033[0m")
            sys.exit(1)
    ...
    if not args.no_static:
        try:
            ...
            print(f"{C.YELLOW}⚠ static_analyzer import failed: {e} — static analysis disabled{C.R}")
            static_analyzer = None
    ...
    if not args.skip_preflight:
        ...
        if not ok:
            print(f"\n{C.RED}{C.BOLD}❌ Pre-flight FAILED. Lütfen yukarıdaki sorunları çöz, sonra tekrar dene.{C.R}")
    ...
    if args.prompts:
        ...
        if not prompts:
            print(f"{C.RED}❌ --prompts ile eşleşen yok: {args.prompts}{C.R}"); sys.exit(1)
    if args.limit:
        prompts = prompts[:args.limit]
    if not prompts:
        print(f"{C.RED}❌ Prompt bulunamadı{C.R}"); sys.exit(1)
    ...
    if not args.no_metadata and not args.resume:
        ...
        try:
            ...
            if r.returncode == 0:
                print(f"  {C.GREEN}✓{C.R} ENVIRONMENT.md, MODEL_METADATA.md, ACADEMIC_HYPOTHESES.md written")
            else:
                print(f"  {C.YELLOW}⚠ academic_metadata.py rc={r.returncode}: {r.stderr[-300:]}{C.R}")
        except Exception as e:
            print(f"  {C.YELLOW}⚠ academic metadata skipped: {e}{C.R}")
    ...
    try:
        for mi, model in enumerate(args.models, 1):
            ...
            for pi, p in enumerate(prompts, 1):
                ...
                try:
                    ...
                    print()
                    if aborted:
                        print(f"  {C.YELLOW}⚠ {abort_reason} — üretim kesildi (uzunluk: {len(raw_response)} char){C.R}")
                except Exception as e:
                    print(f"\n  {C.RED}✗ LLM hata: {e}{C.R}")
                    raw_response = ""
                ...
                if static_analyzer is not None and code:
                    try:
                        ...
                        print(f"  {C.YELLOW}⚠ static analysis failed: {e}{C.R}")
        ...
        print(f"  {C.GREEN}{C.BOLD}✅ Tüm testler tamamlandı{C.R}")
